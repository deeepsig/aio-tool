...
export function useProcessSteps() {
  ...
  const startFetchStep = useCallback(() => {
    // Reset steps and start fresh with only the fetch step
  ...
  const updateFromRobotsResult = useCallback(
    (result: RobotsTxtResult) => {
      if (result.success) {
        // Update fetch step to completed
        ...
        // Update fetch step to error
        updateStep('fetch-robots', {
          status: 'error',
          error: `Error: ${result.error}\n\nTried to fetch from: ${result.url}`,
        });

        // Ensure no analysis step exists when fetch fails
        // Remove any existing analysis step from previous runs
        setSteps((prev) => prev.filter((step) => step.id !== 'analyze-robots'));
      }
    },
    [updateStep, addStep]
  );
